...
float pastImuVals[3] = {0.0, 0.0, 0.0};
int frequency = 0;
...
CircularBuffer<long, 100> magReads[3] = {magReadX, magReadY, magReadZ};
...
void loop() {
  ...
  if ( imu.magAvailable() ) {
    ...
    float curImuVals[3] = {imu.calcMag(imu.mx), imu.calcMag(imu.my),imu.calcMag(imu.mz)};

    for (int i = 0; i < 3; ++i) // 3 axis, X Y Z 
    {
      if (curImuVals[i] * pastImuVals[i] < 0) {
        magReads[i].push(cur_time);
      }
      if (cur_time - magReads[i].first() > 1000) { // 1000 ms
        frequency = magReads[i].size() / 2;
        Serial.print(frequency); Serial.print('\t');
        int err = 3;
        if (frequency > (5 - err) && frequency < (5 + err)){
          Firebase.setString("kiosk/1/object", mac);
          Serial.print("Frequency_Threshold_hit!");
        }
        magReads[i].clear();
      }
      pastImuVals[i] = curImuVals[i];
    }
